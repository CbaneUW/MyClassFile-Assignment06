...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment06DB_CBane')
	 Begin 
	  Alter Database [Assignment06DB_CBane] set Single_user With Rollback Immediate;
	  Drop Database Assignment06DB_CBane;
	 End
	Create Database Assignment06DB_CBane;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment06DB_CBane;
...
WITH SCHEMABINDING
AS
 SELECT ProductID, CategoryID, UnitPrice, ProductName AS [ProdName] 
 FROM dbo.Products;
GO
CREATE --DROP
VIEW vInventories
...
WITH SCHEMABINDING
AS
 SELECT EmployeeID, ManagerID, EmployeeFirstName + ' ' + EmployeeLastName AS [EmployeeName] 
 FROM dbo.Employees;
GO
--End Question 1.
...
GRANT SELECT vProducts TO Public;

USE Employees;
DENY SELECT Employees TO PUBLIC;
GRANT SELECT vEmployees TO Public;
 
USE Inventories;
DENY SELECT Inventories TO PUBLIC;
GRANT SELECT vInventores TO Public;
--End Question 2.
...
SELECT TOP 100000
 ...
 FROM Products FULL OUTER JOIN Inventories
...
AS
SELECT TOP 100000 
 DISTINCT Inventories.InventoryDate, Employees.EmployeeFirstName + ' ' + Employees.EmployeeLastName AS EmployeeName
...
GO
CREATE VIEW vInventoriesByProductsByEmployeesObject
AS
SELECT TOP 100000
...
GO
CREATE VIEW vEmployeesByManagerObject
AS
SELECT ManagerID, EmployeeID, EmployeeFirstName + ' '  + EmployeeLastName as Employee
 FROM Employees

SELECT E.ManagerID, E.EmployeeID, E.EmployeeFirstName + ' ' + E.EmployeeLastName as Employee
 FROM Employees as E
...
Select * From [dbo].[vInventoriesByProductsByEmployeesObject]
Select * From [dbo].[vInventoriesForChaiAndChangByEmployees]
Select * From [dbo].[vEmployeesByManagerObject]
Select * From [dbo].[vInventoriesByProductsByCategoriesByEmployees]
Select * From [dbo].[vQuestion10]
